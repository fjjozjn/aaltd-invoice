...
Class Request{
    ...
    public function submitOrderInfo(){
        ...
        $this->reqHandler->setParameter('service','pay.tenpay.jspay');//接口类型：pay.alipay.jspay
		...
		$this->reqHandler->setParameter('notify_url','http://dawei.dev.swiftpass.cn/payInterface_tenpay_js/request.php?method=callback');//通知地址，商户需改为自己正式的
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    echo json_encode(array('pay_url'=>$this->resHandler->getParameter('pay_url'),
										'pay_info'=>$this->resHandler->getParameter('pay_info')), JSON_UNESCAPED_SLASHES);
                    exit();
                }else{
